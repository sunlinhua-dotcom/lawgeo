...
> = {
  citation_drop: (d) => ({
    subject: `[BrandGEO] ${d.project} AI 引用率下跌告警 ⚠️`,
    html: alertHtml({
      title: "AI 引用率显著下跌",
      ...
      lines: [
        ...
        "建议：检查 BrandGEO 控制台的引用监测页，找出哪些平台跌得最厉害。",
      ],
      cta: "查看监测面板",
  ...
  top3_lost: (d) => ({
    subject: `[BrandGEO] ${d.project} 跌出 AI Top3 推荐`,
    html: alertHtml({
      title: "跌出 Top 3 推荐位置",
      project: String(d.project ?? "—"),
      lines: [
  ...
  competitor_overtake: (d) => ({
    subject: `[BrandGEO] 竞品 ${d.competitor} 在 AI 推荐中反超你`,
    html: alertHtml({
      title: "竞品反超",
      project: String(d.project ?? "—"),
      lines: [
        `<b>${d.competitor}</b> 现在被 AI 推荐的次数已超过你。`,
        "建议：用 BrandGEO 对比工具拉一次同问题的回答，分析竞品获胜的具体原因。",
      ],
      cta: "用对比工具分析",
  ...
  weekly_digest: (d) => ({
    subject: `[BrandGEO] 每周 GEO 数据简报 · ${d.weekRange ?? ""}`,
    html: alertHtml({
      title: "每周 GEO 数据简报",
      project: String(d.project ?? "—"),
      lines: [
...
<tr><td>
  <h1 style="font-size: 24px; margin: 0; color: #6366f1;">BrandGEO · 告警</h1>
...
export async function sendAlert(opts: { to: string; type: AlertType; data: Record<string, unknown> }) {
  ...
  if (!resend) {
    console.log(`[BrandGEO email] (no RESEND_API_KEY, simulated send)\nTo: ${opts.to}\nSubject: ${subject}\n`);
