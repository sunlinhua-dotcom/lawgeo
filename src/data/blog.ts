...
1. 用 [lawGEO 免费诊断工具](/tools/audit) 跑一次自己的域名
2. 按照报告里的优先级建议开始补
3. 同步建立监测，看哪些动作生效
...
- [律所 GEO 实战手册](/blog/lawyer-geo-handbook)
...
- 用 [lawGEO 诊断工具](/tools/audit) 跑一次，确认 llms.txt 项是 ✅
`,
  },
  {
    slug: "lawyer-geo-handbook",
    title: "律所 GEO 实战手册：从 0 到 100 的完整路径",
    description: "律所怎么做 GEO？从案由词库梳理到内容矩阵搭建到引用监测，给你一份可执行的 12 周路线图。",
    category: "律所专题",
    date: "2026-05-26",
    readMinutes: 18,
    content: `# 律所 GEO 实战手册

> **TL;DR**：律所做 GEO 的核心是把案由词库 × 地域矩阵转化为结构化内容矩阵，并通过 12 平台监测持续优化。12 周可完成第一轮闭环。

## 1. 0–4 周：词库与现状

### 词库梳理
- 用 [lawGEO 案由矩阵工具](/tools/matrix) 选 1–3 个主推方向
- 城市优先：你实际能服务的城市
- 案由优先：你团队真实强项 + 客单价 + 案件量

### 现状诊断
- 用 [lawGEO 诊断工具](/tools/audit) 跑当前域名
- 列出缺失的：llms.txt / schema.org / FAQ / 首段直答
- 列出竞品：在 [对比工具](/tools/compare) 跑 5 个核心问题，看 AI 推荐谁

## 2. 5–8 周：内容矩阵

按下面顺序补：

1. **关于页 + 律师团队页**（含 Person schema）
2. **案由 FAQ 页**（每个案由 10–20 个 Q&A，含 FAQPage schema）
3. **收费方式说明页**（保留具体区间）
4. **办案流程页**（HowTo schema）
5. **典型案例页**（脱敏 + 「以个案为准」提示）
6. **案由 × 城市落地页**（按矩阵预生成，每页 800–1500 字）
...
- 持续扩张案由 × 城市矩阵（每月新增 30–50 页）
- 建立用户咨询闭环（让 AI 推荐 → 官网 → 微信咨询）
- 季度做一次完整 GEO 审计

## 5. 合规

- 全程使用 [lawGEO 合规审查模板](/cases/lawyer)
- 避免「最」「第一」「保证胜诉」等绝对化用语
- 案例展示一律脱敏
